...
const resolveResetBaseUrl = () => {
    const url = process.env.RESET_PASSWORD_URL || process.env.PASSWORD_RESET_URL || 'https://accountsybill.com/reset'
    return url.replace(/\/$/, '')
}
...
export const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        const resetLink = `${resolveResetBaseUrl()}/${token}`
        const fromAddress = process.env.SMTP_FROM || `Accountsybill <${USER || 'no-reply@accountsybill.com'}>`
            ...
            html: `
                ...
                <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6; background: #f9f9f9; padding: 20px;">
                    <div style="max-width: 600px; margin: 0 auto; background: #fff; border-radius: 8px; padding: 24px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
                        ...
                        <p style="margin-top: 32px; font-size: 13px; color: #888;">— The Accountsy Invoicing Team</p>
                    </div>
                </body>
                </html>
            `
